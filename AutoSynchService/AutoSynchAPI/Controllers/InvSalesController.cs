...
namespace AutoSynchAPI.Controllers
{
    ...
    public class InvSalesController : Controller
    {
        ...
        [Route("DeleteDups")]
        [HttpGet]
        public IActionResult DeleteDups(int branch_id)
        {
            try
            {
                ...
                for (int i = 1; i <= branch_id; i++)
                {
                    ...
                    for (int j = 6; j <= month; j++)
                    {
                        try
                        {
                            using (Entities dbContext = new Entities())
                            {
                                string qry = @"select count(distinct m.Id) counts,m.FbrInvoiceNumber from InvSaleDetail d inner join 
InvSaleMaster m on d.BillId=m.Id inner join
UsrSystemUser o on m.CompletedBy=o.Id inner join
InvProduct p on d.ProductId=p.Id inner join 
...
where m.FbrInvoiceNumber is not null and b.OrgId=1 and m.IsDeleted=0 and m.OrderStatus<>'KOT'
 ...
 and m.BranchId= " + i + " group by m.FbrInvoiceNumber having count(distinct m.Id)>1";
                                ...
                                {
                                    accOverallMasterIds.ForEach(o => {

                                        var minRecords = dbContext.InvPurchaseMasterMin.FromSqlRaw<InvPurchaseMasterMin>(@"select min(OrderNo) OrderNo from InvSaleMaster
where FbrInvoiceNumber='" + o.FbrInvoiceNumber + "' and BranchId= " + i).FirstOrDefault();
                                        if (minRecords != null)
                                        {
                                            List<InvSaleMaster> invSaleMasters = dbContext.InvSaleMaster.Where(e => e.FbrInvoiceNumber == o.FbrInvoiceNumber && e.OrderNo != minRecords.OrderNo && e.BranchId == i).ToList();
                                            invSaleMasters.ForEach(o => {
                                                o.IsDeleted = true;
                                            });
                                            dbContext.SaveChanges();
                                        }
						...
						order by count(x.id) desc";
                                ...
                                if (accOverallMasterIds != null)
                                {
                                    accOverallMasterIds.ForEach(o => {
                                        ...
                                        var minRecords = dbContext.InvPurchaseMasterMin.FromSqlRaw<InvPurchaseMasterMin>(@"select min(OrderNo) OrderNo from InvSaleMaster
where InvoiceNo=" + o.InvoiceNo + " and FbrInvoiceNumber is null and FiscalYearId=" + o.FiscalYearId + " and BranchId= " + o.BranchId).FirstOrDefault();
                                        if (minRecords != null)
                                        {
                                            List<InvSaleMaster> invSaleMasters = dbContext.InvSaleMaster.Where(e => e.InvoiceNo == o.InvoiceNo && e.FiscalYearId == o.FiscalYearId && e.OrderNo != minRecords.OrderNo && e.BranchId == o.BranchId && e.FbrInvoiceNumber==null).ToList();
                                            invSaleMasters.ForEach(o => {
                                                o.IsDeleted = true;
                                            });
                                            dbContext.SaveChanges();
